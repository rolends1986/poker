...
type game interface {
	...
	ShowBoardCards(deck *hand.Deck, start, end int) []*hand.Card
	SplitPot() bool
...
func (g *holdemGame) ShowBoardCards(deck *hand.Deck, start, end int) []*hand.Card {
	return deck.ShowCards(start, end)
}
...
// TODO: show stuGame board cards
func (g *studGame) ShowBoardCards(deck *hand.Deck, start, end int) []*hand.Card {
	return []*hand.Card{}
}
...
func omahaHands(holeCards []*hand.Card, board []*hand.Card, opts func(*hand.Config)) []*hand.Hand {
	...
	boardSelected := make([]*hand.Card, 3)
	totalBoardCard := len(board)
	...
	for _, indexes := range util.Combinations(4, 2) {
		...
		if totalBoardCard > 0 {
			for _, boardIndexs := range util.Combinations(totalBoardCard, 3) {
				for m, n := range boardIndexs {
					if totalBoardCard > 0 {
						boardSelected[m] = board[n]
					}
				}
				cards := append(boardSelected, selected...)
