...
var tests = []struct {
	...
	{
		G:              Holdem,
		Cards:          pokertest.Cards("Qh", "Ks", "4s", "3d", "4s", "8h", "2c", "Ah", "Kh"),
		NumOfHoleCards: 2,
		NumOfBoard:     5,
	...
	{
		G:              StudHiLo,
		Cards:          pokertest.Cards("Ah", "Ks", "4s", "3d", "5s", "8h", "2c"),
		NumOfHoleCards: 7,
		NumOfBoard:     0,
...
func holdemTable() *Table {
	...
	return New(opts, hand.NewDealer())
}
func omahaHiTable() *Table {
	opts := Config{
		Game: OmahaHi,
		Stakes: Stakes{
			SmallBet: 10,
...
func BenchmarkHoldemShowdown(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		p.payout(0, tbl, highHands, lowHands, hand.SortingHigh, 0)
	}
}

func BenchmarkOmahaHiShowdown(b *testing.B) {
	tbl := omahaHiTable()
	p1 := &player{id: 0}
	p2 := &player{id: 1}
	p3 := &player{id: 2}
	...
	for i := 0; i < b.N; i++ {
		...
		hCards := cardsFromHoleCardMap(holeCards)
		highHands := newHands(hCards, board, tbl.game().FormHighHand)
		lowHands := newHands(hCards, board, tbl.game().FormLowHand)
		payout := p.payout(0, tbl, highHands, lowHands, hand.SortingHigh, 0)
		for _, results := range payout {
			fmt.Println(results)
		}
		fmt.Println("-----------------------------")
	}
